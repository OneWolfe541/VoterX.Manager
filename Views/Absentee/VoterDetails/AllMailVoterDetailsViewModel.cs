...
{
    public class AllMailVoterDetailsViewModel : NotifyPropertyChanged
    {
        public NMVoter Voter { get; set; }

        private bool SERVISSource = false;

        public AllMailVoterDetailsViewModel(NMVoter voter) : this(voter, false) { }
        public AllMailVoterDetailsViewModel(NMVoter voter, bool source)
        {
            Voter = voter;
        ...
        public bool CanPrintBallotBundle
        {
            get
            {
                return Voter.Data.LogCode <= 5 && Voter.IdRequired() != true;
            }
        }
        ...
        private void PrintBallotBundleClick()
        {
            ...
            else if (ElectionDataMethods.Election.AbsenteeHasStarted())
            {
                AlertDialog newMessage = new AlertDialog("THIS ELECTION HAS NOT STARTED");
                newMessage.ShowDialog();
            }
            ...
            else
            {
                ...
                {
                    AlertDialog newAlertMessage = new AlertDialog("A VALID ID IS REQUIRED FOR THIS VOTER");
                    newAlertMessage.ShowDialog();
                    return;
                }
                ...
                if (GetSelectionOrDeliveryAddress(1))
                {
                    ...
                    PrintMailBundleWithoutApplication();
        ...
        public RelayCommand _printIdRequiredBundleCommand;
        public ICommand PrintIdRequiredBundleCommand
        {
            get
            {
                if (_printIdRequiredBundleCommand == null)
                {
                    _printIdRequiredBundleCommand = new RelayCommand(param => this.PrintIdRequiredBundleClick(), param => this.CanPrintIdRequiredBallotBundle);
                }
                return _printIdRequiredBundleCommand;
            }
        }

        // Enable or Disable the Print Ballot Button 
        public bool CanPrintIdRequiredBallotBundle
        {
            get
            {
                return Voter.Data.LogCode <= 5 && Voter.IdRequired() == true;
        ...
        private void PrintIdRequiredBundleClick()
        {
            if (Voter.Data.BallotStyleID == null)
            {
                AlertDialog newMessage = new AlertDialog("INELIGIBLE VOTER");
            ...
            else if (WrongBallotStyle(Voter) == true)
            {
                AlertDialog newMessage = new AlertDialog("WRONG BALLOT STYLE");
                newMessage.ShowDialog();
            }
            else if (Voter.HasRejectedApplication())
            {
            ...
            {
                // Check for ID required (10/28/2019)
                if (Voter.IdRequired() != true)
                {
                    AlertDialog newMessage = new AlertDialog("A VALID ID IS NOT REQUIRED FOR THIS VOTER");
                    newMessage.ShowDialog();
                    return;
                }
                ...
                if (GetSelectionOrDeliveryAddress(1))
                {
                    ...
                    VoterMethods.InsertProvisionalBallot(Voter, 2);
        ...
        private async void PrintMailBundleWithoutApplication_old()
        {
            if (await Task.Run(() => VoterMethods.Exists == true))
            {
                DateTime issuedDate = DateTime.Now;
        ...
        private async void PrintMailBundleWithoutApplication()
        {
            if (await Task.Run(() => VoterMethods.Exists == true))
            {
                DateTime issuedDate = DateTime.Now;
        ...
        private bool WrongBallotStyle(NMVoter Voter)
        {
            bool result = false;

            if (AppSettings.Ballots.IncludeBallotStyleId != 0)
            {
                if (Voter.Data.BallotStyleID != AppSettings.Ballots.IncludeBallotStyleId) result = true;
            }
            else if (AppSettings.Ballots.ExcludeBallotStyleId != 0)
            {
                if (Voter.Data.BallotStyleID == AppSettings.Ballots.ExcludeBallotStyleId) result = true;
            }
        ...
        #region ReissueBallotCommands
        public RelayCommand _printReissueBundleCommand;
        public ICommand PrintReissueBundleCommand
        {
            get
            {
                if (_printReissueBundleCommand == null)
                {
                    _printReissueBundleCommand = new RelayCommand(param => this.PrintReissueBundleClick(), param => this.CanPrintReissueBundle);
                }
                return _printReissueBundleCommand;
            }
        }

        // Enable or Disable the Reissue Button  
        public bool CanPrintReissueBundle
        {
            get
            {
        ...
        private void PrintReissueBundleClick()
        {
            // Check if ballot exists
            if (Voter.HasVoted())
            {
                if (ElectionDataMethods.Election.AbsenteeHasStarted())
                {
                    AlertDialog newMessage = new AlertDialog("THIS ELECTION HAS NOT STARTED");
                    newMessage.ShowDialog();
                }
                else if (Voter.Data.LogCode == 9)
                {
                    AlertDialog newMessage = new AlertDialog("CANNOT REISSUE AN ACCEPTED BALLOT");
                    newMessage.ShowDialog();
                }
                else if (Voter.Data.LogCode == 10)
                {
                    AlertDialog newMessage = new AlertDialog("CANNOT REISSUE A REJECTED BALLOT");
                    newMessage.ShowDialog();
                }
                else if (Voter.Data.LogCode != 6 && Voter.Data.LogCode != 14)
                {
                    AlertDialog newMessage = new AlertDialog("CANNOT REISSUE A BALLOT");
                    newMessage.ShowDialog();
                }
                else if (WrongBallotStyle(Voter) == true)
                {
                    ...
                    if (GetSelectionOrDeliveryAddress(1))
                    {
                        ...
                        if (Voter.IdRequired().Value)
                        {
                            ...
                            if (newMessage.ShowDialog() == true)
                            {
                                try
                                {
                                    ...
                                    if (Voter.GenerateIntelligentBarcodes(imbParameters) == true)
                                    {
                                        // Add provisional record with reason ID Required
                                        VoterMethods.InsertProvisionalBallot(Voter, 2);

                                        // Create Spoiled Record
                                        Voter.SpoilBallot(5);

                                        Voter.UpdateReissued();

                                        PrintReissuedMailBundle();

                                        RemoveVoterFromScanHistory(Voter);
                                    }
                                }
                                catch (Exception e)
                                {
                                    ...
                                    GlobalReferences.StatusBar.TextCenter = (e.Message);
                                    VoterXLogger ABSLogger = new VoterXLogger("ABSLogs", AppSettings.System.ReportErrorLogging);
                                    ABSLogger.WriteLog("Error Reissuing Id Required Ballot: " + e.Message);
                                    if (e.InnerException != null)
                                    {
                                        ABSLogger.WriteLog(e.InnerException.Message);
                                        if (e.InnerException.InnerException != null)
                        ...
                        else
                        {
                            ...
                            catch (Exception e)
                            {
                                ...
                                GlobalReferences.StatusBar.TextCenter = (e.Message);
                                VoterXLogger ABSLogger = new VoterXLogger("ABSLogs", AppSettings.System.ReportErrorLogging);
                                ABSLogger.WriteLog("Error Reissuing Ballot: " + e.Message);
                                if (e.InnerException != null)
                                {
                                    ABSLogger.WriteLog(e.InnerException.Message);
                                    if (e.InnerException.InnerException != null)
            ...
            {
                AlertDialog newMessage = new AlertDialog("THERE IS NO EXISTING BALLOT TO REISSUE");
                newMessage.ShowDialog();
            }
        }

        private async void PrintReissuedMailBundle()
        {
            if (await Task.Run(() => VoterMethods.Exists == true))
            {
                DateTime issuedDate = DateTime.Now;
        ...
        private void RemoveVoterFromScanHistory(NMVoter voter)
        {
            // Remove voter from scan session
            using (var scanHistory = new ScanHistoryFactory())
            {
                scanHistory.RemoveVoter(voter);
            }
        }
        #endregion
